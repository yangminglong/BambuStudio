...
ACTipsDialog::ACTipsDialog(wxWindow *parent, const wxString &title)
    : DPIDialog(parent, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_confirm->SetSize(ACTIPS_DIALOG_BUTTON_SIZE);
    m_confirm->SetMinSize(ACTIPS_DIALOG_BUTTON_SIZE);
    m_confirm->SetCornerRadius(FromDIP(12));
    m_confirm->Bind(wxEVT_LEFT_DOWN, &ACTipsDialog::on_ok, this);
...
wxBoxSizer *ACTipsDialog::create_item_checkbox(wxString title, wxWindow *parent, wxString tooltip, std::string param)
{
...
void ACTipsDialog::on_dpi_changed(const wxRect &suggested_rect)
{
    if (m_confirm) m_confirm->SetMinSize(ACTIPS_DIALOG_BUTTON_SIZE);
    if (m_cancel) m_cancel->SetMinSize(ACTIPS_DIALOG_BUTTON_SIZE);
    Fit();
    Refresh();
}

void ACTipsDialog::on_ok(wxMouseEvent &event)
{
    if (m_show_again) {
...
void ACParamsPanel::Highlighter::set_timer_owner(wxEvtHandler *owner, int timerid /* = wxID_ANY*/)
{
    m_timer.SetOwner(owner, timerid);
}

void ACParamsPanel::Highlighter::init(std::pair<wxStaticBitmap *, bool *> params, wxWindow *parent)
    {
...
void ACParamsPanel::Highlighter::invalidate()
{
    m_timer.Stop();
...
void ACParamsPanel::Highlighter::blink()
{
    if (m_bitmap && m_show_blink_ptr) {
...
ACParamsPanel::ACParamsPanel( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name )
    : wxPanel( parent, id, pos, size, style, name )
{
    ...
    if (m_mode_region)
        m_mode_region->Bind(wxEVT_TOGGLEBUTTON, &ACParamsPanel::OnToggled, this);
    if (m_mode_view)
        m_mode_view->Bind(wxEVT_TOGGLEBUTTON, &ACParamsPanel::OnToggled, this);
    Bind(wxEVT_TOGGLEBUTTON, &ACParamsPanel::OnToggled, this); // For Tab's mode switch
...
void ACParamsPanel::init_bitmaps()
{
    m_tips_arrow_icon = create_scaled_bitmap("tips_arrow", nullptr, 24);
}

void ACParamsPanel::create_layout()
{
#ifdef __WINDOWS__
...
void ACParamsPanel::rebuild_panels()
{
    refresh_tabs();
    free_sizers();
    create_layout();
...
void ACParamsPanel::refresh_tabs()
{
...
void ACParamsPanel::OnToggled(wxCommandEvent& event)
{
...
void ACParamsPanel::set_active_tab(wxPanel* tab)
{
...
bool ACParamsPanel::is_active_and_shown_tab(wxPanel* tab)
{
...
void ACParamsPanel::update_mode()
{
...
void ACParamsPanel::msw_rescale()
{
...
void ACParamsPanel::switch_to_global()
{
    m_mode_region->SetValue(false);
    set_active_tab(nullptr);
}

void ACParamsPanel::switch_to_object(bool with_tips)
{
...
void ACParamsPanel::free_sizers()
{
    if (m_top_sizer)
    {
...
void ACParamsPanel::delete_subwindows()
{
    if (m_title_label)
    {
